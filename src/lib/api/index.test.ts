...
describe('API functions', () => {
  ...
  describe('Auth', () => {
    ...
    it('postOAuthCallback calls post', async () => {
      mockPost.mockResolvedValueOnce({ userId: 'u1', channelName: 'Ch', plan: 'free' });
  ...
  describe('Generate', () => {
    ...
    it('getHistory fetches paginated results', async () => {
      mockGet.mockResolvedValueOnce({ jobs: [], totalCount: 0, limit: 20, offset: 0 });
      await getHistory(20, 0);
      expect(mockGet).toHaveBeenCalledWith('/generate/history?limit=20&offset=0');
    });
  });
  ...
  describe('Download', () => {
    it('getDownloadURL returns URL', async () => {
      mockGet.mockResolvedValueOnce({ downloadUrl: 'https://s3.example.com/dl' });
