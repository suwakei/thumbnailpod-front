...
interface EditorCanvasProps {
  ...
  onObjectModified: (layerId: string, type: string, data: Record<string, unknown>) => void;
...
const EditorCanvas = forwardRef<CanvasRef, EditorCanvasProps>(function EditorCanvas(
  { layers, activeTool, zoom, onObjectSelected, onObjectModified, onZoomChange },
  ref,
) {
  ...
  useEffect(() => {
    ...
    canvas.on("selection:created", (e) => {
      ...
      if (obj) {
        onObjectSelected((obj as unknown as { layerId?: string }).layerId ?? null);
      }
    });
      ...
      for (const layer of layers) {
        if (loadedLayersRef.current.has(layer.id)) {
          ...
          const existing = objects.find(
            (obj) => (obj as unknown as { layerId?: string }).layerId === layer.id,
          );
          if (existing) {
        ...
        try {
          const img = await FabricImage.fromURL(layer.url, { crossOrigin: "anonymous" });
  ...
  const addText = useCallback(
    (text: string, options?: Partial<FabricTextOptions>) => {
      ...
      (textObj as unknown as { layerLabel: string }).layerLabel = `Text: ${text.slice(0, 20)}`;
